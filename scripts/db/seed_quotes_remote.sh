#!/bin/bash
# Seed quote data into a remote Supabase project via Management API
set -euo pipefail
...
echo "==> Cleaning existing quote data..."
run_sql "DELETE FROM public.quote_day_overrides; DELETE FROM public.quotes; DELETE FROM public.quote_sets;"

echo "==> Inserting 10 quote sets..."
run_sql "
...
ON CONFLICT (quote_set_id, locale) DO NOTHING;
"

echo "==> Inserting Russian quotes..."
run_sql "
INSERT INTO public.quotes (quote_set_id, locale, text, source, draft) VALUES
...
echo "==> Verifying..."
run_sql "SELECT (SELECT count(*) FROM public.quote_sets) as sets, (SELECT count(*) FROM public.quotes) as quotes;"
echo ""
echo "Done!"
