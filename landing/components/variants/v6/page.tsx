...
const translations: Record<SupportedLocale, any> = {
  ...
  ru: {
    hero: {
      ...
      title: "Ваша религия\nв фокусе.",
      subtitle:
        "Офлайн-трекер исламских привычек. Создан для регулярности и духовного роста.",
    },
    features: [
      {
        id: "today",
  ...
  kz: {
    hero: {
      ...
      title: "Дініңіз,\nәдемі қадағаланады.",
      subtitle:
        "Назар аударуға, тұрақтылыққа және рухани дамуға арналған офлайн исламдық әдет трекері.",
    },
    features: [
      {
        id: "today",
...
function WaitlistCounter({ text, isDark }: { text: string; isDark: boolean }) {
  ...
  return (
    <div
      className={`flex items-center gap-3 text-sm justify-center mb-6 transition-colors duration-500 ${isDark ? "text-white/70" : "text-black/60"}`}
    >
      <div className="flex -space-x-2">
        <div
          className={`w-7 h-7 rounded-full bg-gradient-to-br from-blue-400 to-indigo-500 border-2 z-30 ${isDark ? "border-black" : "border-[#FAFAFA]"}`}
        />
        <div
          className={`w-7 h-7 rounded-full bg-gradient-to-br from-emerald-400 to-teal-500 border-2 z-20 ${isDark ? "border-black" : "border-[#FAFAFA]"}`}
        />
        <div
...
function WaitlistForm({
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    const res = await submitWaitlist({
      email,
      turnstileToken: token || "demo-token",
      locale,
      variant,
    });
    setStatus(
  ...
  const inputBg = isDark
    ? "bg-white/5 border-white/10 text-white placeholder:text-white/30 focus:border-[#00ff88]/50 focus:bg-white/10"
    : "bg-black/5 border-black/10 text-black placeholder:text-black/40 focus:border-[#00aa55]/50 focus:bg-black/10";
  const btnBg = isDark
    ? "bg-white text-black hover:bg-[#00ff88]"
    : "bg-black text-white hover:bg-[#00aa55]";
  const successBg = isDark
    ? "bg-[#00ff88]/10 border-[#00ff88]/20 text-[#00ff88]"
    : "bg-[#00aa55]/10 border-[#00aa55]/20 text-[#00aa55]";
...
export default function V6Page({ locale: initialLocale, variant }: Props) {
  ...
  const switchLocale = () => {
    const next =
      activeLocale === "en" ? "ru" : activeLocale === "ru" ? "kz" : "en";
  ...
  return (
    <div
      className={`min-h-screen transition-colors duration-700 ease-in-out font-sans ${rootClass}`}
    ...
    >
      ...
      <section className="relative min-h-[100dvh] flex flex-col items-center justify-center px-6 pt-32 pb-20 overflow-hidden">
        {/* Glow */}
        <div
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] pointer-events-none opacity-40 mix-blend-screen transition-opacity duration-1000"
          style={{
            background: `radial-gradient(circle at center, ${isDark ? "rgba(0,255,136,0.12)" : "rgba(0,170,85,0.06)"} 0%, transparent 60%)`,
          }}
        />
        ...
        >
          <div
            className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full border mb-8 transition-colors ${isDark ? "bg-white/5 border-white/10" : "bg-black/5 border-black/10"}`}
          >
            <span
            ...
            />
            <span
              className={`text-xs font-semibold tracking-widest uppercase ${mutedTextClass}`}
            >
              {t.hero.badge}
            </span>
          </div>
          ...
          <p
            className={`text-lg md:text-2xl max-w-2xl mx-auto font-light leading-relaxed mb-16 ${mutedTextClass}`}
          >
            {t.hero.subtitle}
          </p>
      ...
      <section ref={containerRef} className="relative h-[400vh]">
        <div
          className={`sticky top-0 h-screen flex items-center justify-center overflow-hidden border-t transition-colors ${borderClass}`}
        >
          ...
          <div
            ...
            style={{
              ...
              transform: `translate(-50%, -50%) scale(${1 + activeFeature * 0.1})`,
            }}
          />
          ...
          <div className="absolute left-6 md:left-16 lg:left-32 top-32 md:top-1/2 md:-translate-y-1/2 w-[calc(100%-48px)] md:w-[340px] lg:w-[420px] z-20 text-center md:text-left">
            <AnimatePresence mode="wait">
              ...
              >
                <div
                  className={`text-xs tracking-[0.2em] font-medium mb-4 transition-colors uppercase`}
                ...
                <p
                  className={`text-lg md:text-xl font-light leading-relaxed ${mutedTextClass}`}
                >
      ...
      {/* Footer Waitlist CTA */}
      <section
        className={`relative py-32 px-6 border-t transition-colors ${borderClass}`}
      >
      ...
      {/* Footer */}
      <footer
        className={`py-10 text-center border-t transition-colors ${borderClass}`}
      >
        <div
          className={`flex flex-col md:flex-row items-center justify-center gap-6 md:gap-12 text-sm ${mutedTextClass}`}
        >
          <Link
